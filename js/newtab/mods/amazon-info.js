(function () {
	const DISPLAY_POPUP_TIMEOUT = 700;
	...
	AmazonInfoPopup.prototype.build = function () {
		...
		const self = this;
		const oldPopup = document.getElementById('amazon-info-popup');
	...
	AmazonInfoPopup.prototype.attach = function (elem) {
		const rect = fvdSpeedDial.Utils.getBoundingClientRect(elem);
		let position = 'right';
	...
	AmazonInfoPopup.prototype.show = function (elem) {
		...
		if (needAnimation) {
			this.elem.style.animationName
				= 'amazon-info-popup-appear-arrow-' + this.elem.getAttribute('arrow');
		}
	};
	...
	let amazonInfoPopupInstance = null;
	const urlAsinCache = {};
	...
	function getAsinByUrl(url) {
		if (!urlAsinCache.hasOwnProperty(url)) {
			...
			try {
				...
				let preparedUrl = url.replace(/^https?:\/\//, '');
				...
				for (var i = 0; i != regexps.length; i++) {
					const m = preparedUrl.match(regexps[i]);
	...
	function onMouseOver(event) {
		const dial = getDialElem(event.target);
		...
		if (dial) {
			...
			const dialUrl = dial.getAttribute('data-url');
			const asin = getAsinByUrl(dialUrl);
			...
			turnOffTitle(dial);
			const popup = getAmazonInfoPopup();
			const show = function () {
	...
	window.addEventListener(
		...
		function (event) {
			...
			if (data.action === 'amazon-info-size' && amazonInfoPopupInstance) {
				...
				data.action
				&& data.action.indexOf('amazon-info.') === 0
				&& amazonInfoPopupInstance
			) {
				const action = data.action.replace('amazon-info.', '');
