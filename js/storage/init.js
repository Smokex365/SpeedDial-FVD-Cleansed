...
const initStorage = function (fvdSpeedDial, tx, databaseBackup, callback) {
	...
	Utils.Async.chain([
		...
		function (callback2) {
			console.info('CREATING Dexie DB', 1, tablesCreated, databaseBackup);
			...
			/*
			if (
				tablesCreated.indexOf('groups') !== -1 &&
				tablesCreated.indexOf('dials') !== -1 &&
				databaseBackup
			) {
				console.info('backup', buckup);
				...
				databaseBackup.restore(tx, function (err) {
					if (err) {
						AppLog.info("! Backup hasn't been restored", err.message);
						console.info("Backup hasn't been restored(probably everything is ok)", err.message);
					} else {
						AppLog.info('Important! SpeedDial database has been restored.');
						backupRestored = true;
					}
			...
			Utils.Async.chain([
				...
				function (next) {
					const processDial = function (dialData, done) {
						...
						if (!dialData.group_id) dialData.group_id = 'default'; //"default"
