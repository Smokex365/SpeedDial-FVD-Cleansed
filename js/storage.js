								...
								ThumbMaker.getImageDataPath(
									{
										...
										screenWidth: 364, // SpeedDial.getMaxCellWidth(), // TODO
									},
		...
		Utils.Async.chain([
			...
			function (next) {
				...
				if (data.thumb.indexOf('data:image/png;base64,https://') !== -1) {
					console.info('Data thumb fix', data.thumb);
						...
						if (typeof webkitRequestFileSystem === 'undefined') {
							...
							FileSystemSD.write(thumbName, thumb, function (err, url) {
								...
								if (FileSystemSD.hasOwnProperty('deleteBlobUrl')) {
									...
									console.info('FileSystemSD.deleteBlobUrl');
								}
							});
						}
					},
			...
			dbSelect(fvdSpeedDial, {
				...
				success: function (tx, results) {
					...
					if (
						...
						// 	false
						// 	&& !tabs[0].incognito
						// 	&& String(tabs[0].cookieStoreId).indexOf('firefox-container-') === -1
						// ) {
						// 	FileSystemSD.getBlobURLbyURL(v, function (par, blobURL) {
						// 		callback(blobURL);
						// 	});
						// } else {
						FileSystemSD.readAsDataURLbyURL(v, function (par, url) {
							// console.log('getMisc read URL', v, par, url);
							callback(url);
						});
						// }
						// });
...
function dbSelect(fvdSpeedDial, obj) {
	if (typeof fvdSpeedDial === 'undefined') {
		console.info('dbSelect', 'fvdSpeedDial', fvdSpeedDial);
	}
...
function dbWhereTest(exp, val, line) {
	...
	if (
		...
		|| (more // ">"
			&& (!exist || parseInt(line[wClean]) <= parseInt(val))
			&& (!equal || !exist || parseInt(line[wClean]) !== parseInt(val))) // "="
		|| (less // "<"
			&& (!exist || parseInt(line[wClean]) >= parseInt(val))
			&& (!equal || !exist || parseInt(line[wClean]) !== parseInt(val))) // "="
		|| (like // "~ LIKE"
